function cfCastbars.SetupSettings()
	...
	plText:SetPoint("TOPLEFT", plEnabled, "BOTTOMLEFT", 0, -5)
	local plTimer = F.Checkbox(sc, K.PlayerTimer, "Timer", pl)
	plTimer:SetPoint("LEFT", plText, "RIGHT", 60, 0)
	local plBorder = F.Checkbox(sc, K.PlayerBorder, "Border", pl)
	plBorder:SetPoint("TOPLEFT", plText, "BOTTOMLEFT", 0, -5)
	...
	plhText:SetPoint("TOPLEFT", plIconY, "BOTTOMLEFT", 0, -15)
	...
	local plFontOutline = F.Dropdown(sc, K.PlayerTextFontOutline, "Font Outline", OUTLINE_ITEMS, pl)
	plFontOutline:SetPoint("TOPLEFT", plTextY, "BOTTOMLEFT", -13, -15)

	local plhTimer = F.Text(sc, "Timer", "GameFontNormal")
	plhTimer:SetPoint("TOPLEFT", plFontOutline, "BOTTOMLEFT", 13, -5)
	...
	local plTimerFontOutline = F.Dropdown(sc, K.PlayerTimerFontOutline, "Font Outline", OUTLINE_ITEMS, pl)
	plTimerFontOutline:SetPoint("TOPLEFT", plTimerY, "BOTTOMLEFT", -13, -15)

	local plReset = F.Button(sc, "Reset", function() F.ResetPrefix("Player", pl) end)
	plReset:SetPoint("TOPLEFT", plTimerFontOutline, "BOTTOMLEFT", 13, -5)

	F.BindChildren(plIcon, {plhIcon, plIconScale, plIconX, plIconY})
	F.BindChildren(plTimer, {plhTimer, plTimerScale, plTimerX, plTimerY, plTimerFontOutline})
	F.BindChildren(plText, {plhText, plTextScale, plTextX, plTextY, plFontOutline})
	F.BindChildren(plEnabled, {plIcon, plTimer, plText, plBorder, plSpark, plShield, plFlash, plhCb, plScale, plX, plY, plW, plH, plhIcon, plIconScale, plIconX, plIconY, plhTimer, plTimerScale, plTimerX, plTimerY, plTimerFontOutline, plhText, plTextScale, plTextX, plTextY, plFontOutline})
	-----------------------------------------------------------------------
	-- Column 2: Target
	-----------------------------------------------------------------------
	...
	tgText:SetPoint("TOPLEFT", tgEnabled, "BOTTOMLEFT", 0, -5)
	local tgTimer = F.Checkbox(sc, K.TargetTimer, "Timer", tg)
	tgTimer:SetPoint("LEFT", tgText, "RIGHT", 60, 0)
	local tgBorder = F.Checkbox(sc, K.TargetBorder, "Border", tg)
	tgBorder:SetPoint("TOPLEFT", tgText, "BOTTOMLEFT", 0, -5)
	...
	tghText:SetPoint("TOPLEFT", tgIconY, "BOTTOMLEFT", 0, -15)
	...
	local tgFontOutline = F.Dropdown(sc, K.TargetTextFontOutline, "Font Outline", OUTLINE_ITEMS, tg)
	tgFontOutline:SetPoint("TOPLEFT", tgTextY, "BOTTOMLEFT", -13, -15)

	local tghTimer = F.Text(sc, "Timer", "GameFontNormal")
	tghTimer:SetPoint("TOPLEFT", tgFontOutline, "BOTTOMLEFT", 13, -5)
	...
	local tgTimerFontOutline = F.Dropdown(sc, K.TargetTimerFontOutline, "Font Outline", OUTLINE_ITEMS, tg)
	tgTimerFontOutline:SetPoint("TOPLEFT", tgTimerY, "BOTTOMLEFT", -13, -15)

	local tgReset = F.Button(sc, "Reset", function() F.ResetPrefix("Target", tg) end)
	tgReset:SetPoint("TOPLEFT", tgTimerFontOutline, "BOTTOMLEFT", 13, -5)
	...
	ptText:SetPoint("TOPLEFT", ptEnabled, "BOTTOMLEFT", 0, -5)
	local ptTimer = F.Checkbox(sc, K.PetTimer, "Timer", pt)
	ptTimer:SetPoint("LEFT", ptText, "RIGHT", 60, 0)
	local ptBorder = F.Checkbox(sc, K.PetBorder, "Border", pt)
	ptBorder:SetPoint("TOPLEFT", ptText, "BOTTOMLEFT", 0, -5)
	...
	pthText:SetPoint("TOPLEFT", ptIconY, "BOTTOMLEFT", 0, -15)
	...
	local ptFontOutline = F.Dropdown(sc, K.PetTextFontOutline, "Font Outline", OUTLINE_ITEMS, pt)
	ptFontOutline:SetPoint("TOPLEFT", ptTextY, "BOTTOMLEFT", -13, -15)

	local pthTimer = F.Text(sc, "Timer", "GameFontNormal")
	pthTimer:SetPoint("TOPLEFT", ptFontOutline, "BOTTOMLEFT", 13, -5)
	...
	local ptTimerFontOutline = F.Dropdown(sc, K.PetTimerFontOutline, "Font Outline", OUTLINE_ITEMS, pt)
	ptTimerFontOutline:SetPoint("TOPLEFT", ptTimerY, "BOTTOMLEFT", -13, -15)

	local ptReset = F.Button(sc, "Reset", function() F.ResetPrefix("Pet", pt) end)
	ptReset:SetPoint("TOPLEFT", ptTimerFontOutline, "BOTTOMLEFT", 13, -5)

	F.BindChildren(ptIcon, {pthIcon, ptIconScale, ptIconX, ptIconY})
	F.BindChildren(ptTimer, {pthTimer, ptTimerScale, ptTimerX, ptTimerY, ptTimerFontOutline})
	F.BindChildren(ptText, {pthText, ptTextScale, ptTextX, ptTextY, ptFontOutline})
	F.BindChildren(ptEnabled, {ptIcon, ptTimer, ptText, ptBorder, ptSpark, ptShield, ptFlash, pthCb, ptScale, ptX, ptY, ptW, ptH, pthIcon, ptIconScale, ptIconX, ptIconY, pthTimer, ptTimerScale, ptTimerX, ptTimerY, ptTimerFontOutline, pthText, ptTextScale, ptTextX, ptTextY, ptFontOutline})
	-----------------------------------------------------------------------
	-- Row 1 separators
	-----------------------------------------------------------------------
	...
	paText:SetPoint("TOPLEFT", paEnabled, "BOTTOMLEFT", 0, -5)
	local paTimer = F.Checkbox(sc, K.PartyTimer, "Timer", pa)
	paTimer:SetPoint("LEFT", paText, "RIGHT", 60, 0)
	local paBorder = F.Checkbox(sc, K.PartyBorder, "Border", pa)
	paBorder:SetPoint("TOPLEFT", paText, "BOTTOMLEFT", 0, -5)
	...
	pahText:SetPoint("TOPLEFT", paIconY, "BOTTOMLEFT", 0, -15)
	...
	local paFontOutline = F.Dropdown(sc, K.PartyTextFontOutline, "Font Outline", OUTLINE_ITEMS, pa)
	paFontOutline:SetPoint("TOPLEFT", paTextY, "BOTTOMLEFT", -13, -15)

	local pahTimer = F.Text(sc, "Timer", "GameFontNormal")
	pahTimer:SetPoint("TOPLEFT", paFontOutline, "BOTTOMLEFT", 13, -5)
	...
	local paTimerFontOutline = F.Dropdown(sc, K.PartyTimerFontOutline, "Font Outline", OUTLINE_ITEMS, pa)
	paTimerFontOutline:SetPoint("TOPLEFT", paTimerY, "BOTTOMLEFT", -13, -15)

	local paReset = F.Button(sc, "Reset", function() F.ResetPrefix("Party", pa) end)
	paReset:SetPoint("TOPLEFT", paTimerFontOutline, "BOTTOMLEFT", 13, -5)

	F.BindChildren(paIcon, {pahIcon, paIconScale, paIconX, paIconY})
	F.BindChildren(paTimer, {pahTimer, paTimerScale, paTimerX, paTimerY, paTimerFontOutline})
	F.BindChildren(paText, {pahText, paTextScale, paTextX, paTextY, paFontOutline})
	F.BindChildren(paEnabled, {paIcon, paTimer, paText, paBorder, paSpark, paShield, paFlash, pahCb, paScale, paX, paY, paW, paH, pahIcon, paIconScale, paIconX, paIconY, pahTimer, paTimerScale, paTimerX, paTimerY, paTimerFontOutline, pahText, paTextScale, paTextX, paTextY, paFontOutline})
	-----------------------------------------------------------------------
	...
	npText:SetPoint("TOPLEFT", npEnabled, "BOTTOMLEFT", 0, -5)
	local npTimer = F.Checkbox(sc, K.NameplateTimer, "Timer", np)
	npTimer:SetPoint("LEFT", npText, "RIGHT", 60, 0)
	local npBorder = F.Checkbox(sc, K.NameplateBorder, "Border", np)
	npBorder:SetPoint("TOPLEFT", npText, "BOTTOMLEFT", 0, -5)
	...
	nphText:SetPoint("TOPLEFT", npIconY, "BOTTOMLEFT", 0, -15)
	...
	local npFontOutline = F.Dropdown(sc, K.NameplateTextFontOutline, "Font Outline", OUTLINE_ITEMS, np)
	npFontOutline:SetPoint("TOPLEFT", npTextY, "BOTTOMLEFT", -13, -15)

	local nphTimer = F.Text(sc, "Timer", "GameFontNormal")
	nphTimer:SetPoint("TOPLEFT", npFontOutline, "BOTTOMLEFT", 13, -5)
	...
	local npTimerFontOutline = F.Dropdown(sc, K.NameplateTimerFontOutline, "Font Outline", OUTLINE_ITEMS, np)
	npTimerFontOutline:SetPoint("TOPLEFT", npTimerY, "BOTTOMLEFT", -13, -15)

	local npReset = F.Button(sc, "Reset", function() F.ResetPrefix("Nameplate", np) end)
	npReset:SetPoint("TOPLEFT", npTimerFontOutline, "BOTTOMLEFT", 13, -5)

	F.BindChildren(npIcon, {nphIcon, npIconScale, npIconX, npIconY})
	F.BindChildren(npTimer, {nphTimer, npTimerScale, npTimerX, npTimerY, npTimerFontOutline})
	F.BindChildren(npText, {nphText, npTextScale, npTextX, npTextY, npFontOutline})
	F.BindChildren(npEnabled, {npIcon, npTimer, npText, npBorder, npSpark, npShield, npFlash, nphCb, npScale, npX, npY, npW, npH, nphIcon, npIconScale, npIconX, npIconY, nphTimer, npTimerScale, npTimerX, npTimerY, npTimerFontOutline, nphText, npTextScale, npTextX, npTextY, npFontOutline})
	-----------------------------------------------------------------------
	-- Row 2 separator
	-----------------------------------------------------------------------
